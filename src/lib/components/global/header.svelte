<script>
	...
	import ShuffleText from '../common/ShuffleText.svelte';
</script>

<header class="relative z-10">
	<div class="w-full m-auto max-w-7xl px-2 sm:px-6 lg:px-8">
		<div class="flex items-center flex-wrap py-6">
			<a href="/" aria-hidden="true" aria-label="logo" class="inline-flex items-center">
			...
			<nav class="ml-auto">
				<ul class="flex flex-col md:flex-row gap-8">
					{#each navigation as { name, path }}
						<li>
							<ShuffleText text={name} href={path} />
						</li>
					{/each}
				</ul>
			</nav>
			<div class="order-last ml-4"></div>
		</div>
	</div>
</header>
